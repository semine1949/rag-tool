...
export const adminApi = {
  ...
  async updateTenantStatus(_id: number, _status: Tenant['status']): Promise<Tenant> {
    throw new Error('后端未提供租户状态切换接口，请通过成员角色管理调整');
  },

  async deleteTenant(_id: number): Promise<void> {
  ...
  async updateUserStatus(_id: number, _status: UserItem['status']): Promise<UserItem> {
  ...
  async deleteKb(_id: number): Promise<void> {
  ...
  async deleteDoc(_id: number): Promise<void> {
    throw new Error('后端未提供单文档删除接口');
  },

  /** 重建索引：后端为知识库维度 reprocess，文档维度暂以知识库级重处理代理 */
  async reprocessDoc(_id: number): Promise<{ id: number }> {
